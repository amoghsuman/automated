...
for i in range(1,len(data)):
    if(i<90):
        ...
        highestTillNow['Sensex'].append(highestTillEmaIsPositive)
        continue
    df = data[:i]
    new_df = df[-90:]
    if(len(new_df)<1):
        continue
    exponential_moving_avg_50_period = new_df['close'].ewm(span=50,min_periods=50).mean()
    ema_slope = slope(exponential_moving_avg_50_period, 5)
    
    if ema_slope[len(ema_slope)-1]<0 and flag==1:
        flag = 2
        buy_point = highestTillEmaIsPositive
        Amount_infused['Sensex'].append(0)
        Commulative_Amount_Infused['Sensex'].append(amount_infused_tillnow)
        ETFs_bought['Sensex'].append(0)
        Current_Investment_Value['Sensex'].append(data['close'][i]*shares_bought_till_now)
    ...
    elif  ema_slope[len(ema_slope)-1]>0 and flag == 2:
        flag = 1
        amount_to_invest=abs((buy_point - data['close'][i])/buy_point)*10000000
        shares_bought=int(amount_to_invest/data['close'][i])
        highestTillEmaIsPositive = data['close'][i]
        if(shares_bought>0):
        ...
        if(ema_slope[len(ema_slope)-1]>0):
            flag = 1
            highestTillEmaIsPositive = max(highestTillEmaIsPositive, data['close'][i])
        Amount_infused['Sensex'].append(0)
        Commulative_Amount_Infused['Sensex'].append(amount_infused_tillnow)
        ETFs_bought['Sensex'].append(0)
        Current_Investment_Value['Sensex'].append(data['close'][i]*shares_bought_till_now)
...
data['NAV'] = np.array(NAV['Sensex'])
data['HighestTillNow'] = np.array(highestTillNow['Sensex'])
data.sort_index(inplace=True,ascending=False)
data.to_csv('Nifty50_50EMA.csv')
