...
for i in range(1,len(data)):
    ...
    df = MACD(new_df, 12, 26, 9) 
    histogram = df['MACD'][len(df)-1] - df['Signal'][len(df)-1]
    if(len(df)<1):
        continue
    ...
    if  macd_slope[len(macd_slope)-1]>0 and histogram>5 and flag==0:
        flag = 1
        ...
        if(shares_bought>0):
            ...
            #Current_Investment_Value['Sensex'].append(data['close'][i]*shares_bought_till_now)
            #NAV['Sensex'].append(current_nav)
    elif (ema_10[len(ema_10)-1] < ema_50[len(ema_50)-1] and ema_10[len(ema_10)-2] > ema_50[len(ema_50)-2]) and flag==1:
        flag=0
